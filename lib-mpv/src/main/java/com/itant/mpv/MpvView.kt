...
open class MpvView(context: Context, attrs: AttributeSet) : BaseMPVView(context, attrs) {
    override fun initOptions() {
        ...
        // vo
        setVo(if (sharedPreferences.getBoolean("gpu_next", false)) "gpu-next" else "gpu")

        // hwdec
        val hwdec = if (sharedPreferences.getBoolean("hardware_decoding", true)) "auto" else "no"
    ...
    fun getRepeat(): Int {
        return when (MPVLib.getPropertyString("loop-playlist") + MPVLib.getPropertyString("loop-file")) {
            "noinf" -> 2
            "infno" -> 1
            else -> 0
        }
